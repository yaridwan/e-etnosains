<x-layout-auth judul-seo="Menunggu Verifikasi">
    <div class="text-center">
        ...
        <h1 class="text-xl font-bold text-slate-900 dark:text-slate-100">Akun Anda Sedang Diverifikasi</h1>

        @if($pengguna->status_akun->value === 'ditolak')
            <x-alert jenis="bahaya" class="mt-4 text-left">
                Pendaftaran Anda ditolak.
                @if($pengguna->catatan_verifikasi)
                    Alasan: {{ $pengguna->catatan_verifikasi }}
                @endif
            </x-alert>
        @elseif($pengguna->status_akun->value === 'nonaktif')
            <x-alert jenis="peringatan" class="mt-4 text-left">
                Akun Anda telah dinonaktifkan oleh Administrator.
            </x-alert>
        @else
            <p class="mt-3 text-sm text-slate-500 dark:text-slate-400">
                Terima kasih telah mendaftar sebagai siswa. Administrator sedang meninjau data Anda.
