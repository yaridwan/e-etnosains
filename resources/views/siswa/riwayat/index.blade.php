<x-layout-dashboard judul-seo="Riwayat Belajar" :menu="\App\Support\MenuDashboard::siswa()">
    <h1 class="text-2xl font-bold text-slate-900">Riwayat Belajar</h1>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($riwayat as $item)
                        <tr>
                            <td class="py-3 font-medium text-slate-700">{{ $item->modelKonten->judul ?? '-' }} <span class="text-xs text-slate-400">({{ $item->labelJenis }})</span></td>
                            <td class="py-3 text-slate-500">{{ $item->persentase_baca }}%</td>
                            <td class="py-3"><x-badge :warna="$item->status === 'selesai' ? 'emerald' : 'amber'">{{ ucfirst($item->status) }}</x-badge></td>
                            <td class="py-3 text-slate-500">{{ $item->terakhir_diakses_pada?->translatedFormat('d M Y, H:i') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
