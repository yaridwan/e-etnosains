<x-layout-auth judul-seo="Masuk">
    ...
    <form method="POST" action="{{ route('masuk.proses') }}" class="mt-6 space-y-4">
        ...
        <div>
            <label for="jawaban_captcha" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">
            ...
            <div class="flex items-stretch gap-3">
                ...
                <input
                    ...
                    @if($errors->has('jawaban_captcha')) aria-invalid="true" @endif
                    class="block w-full rounded-lg border px-3.5 py-2.5 text-sm shadow-sm transition placeholder:text-slate-400 focus:outline-none focus:ring-2 dark:bg-slate-900 dark:text-slate-100 {{ $errors->has('jawaban_captcha')
