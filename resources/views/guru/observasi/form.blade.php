                ...
                <div class="mt-4 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1 space-y-3">
                            <input type="text" :name="`butir[${i}][pertanyaan]`" x-model="item.pertanyaan" placeholder="Tulis pertanyaan..." class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3.5 py-2 text-sm" required>

                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <select :name="`butir[${i}][tipe_pertanyaan]`" x-model="item.tipe_pertanyaan" class="rounded-lg border border-slate-300 dark:border-slate-700 px-3.5 py-2 text-sm">
                                ...
                                <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                                    ...
                                    <input type="checkbox" :name="`butir[${i}][wajib]`" value="1" x-model="item.wajib" class="rounded border-slate-300 dark:border-slate-700 text-teal-700 dark:text-teal-400">
                                    Wajib diisi
                                </label>
                            </div>
                            ...
                            <div x-show="item.tipe_pertanyaan === 'pilihan_tunggal' || item.tipe_pertanyaan === 'pilihan_ganda'" x-cloak>
                                ...
                                <textarea :name="`butir[${i}][opsi_teks]`" rows="3" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3.5 py-2 text-sm" x-init="$el.value = item.opsi.join('\n')" @input="item.opsi = $el.value.split('\n')"></textarea>
                                <template x-for="(opsi, j) in item.opsi" :key="j">
                                    <input type="hidden" :name="`butir[${i}][opsi][${j}]`" :value="opsi">
                                </template>
                            </div>
                        </div>
                        <button type="button" @click="hapus(i)" class="text-sm font-medium text-rose-600 dark:text-rose-400 hover:underline">Hapus</button>
                    </div>
                </div>
            </template>
