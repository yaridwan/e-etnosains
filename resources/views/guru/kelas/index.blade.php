<x-layout-dashboard judul-seo="Kelas Saya" :menu="\App\Support\MenuDashboard::guru()">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-900">Kelas Belajar Saya</h1>
        ...
        @else
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($kelas as $item)
                    <a href="{{ route('guru.kelas.show', $item) }}" class="block rounded-xl border border-slate-200 p-4 hover:border-teal-600">
                        <p class="font-semibold text-slate-800">{{ $item->nama_kelas }}</p>
                        <p class="mt-1 text-xs text-slate-400">Kode: {{ $item->kode_kelas }}</p>
                        <p class="mt-2 text-sm text-slate-500">{{ $item->anggota_count }} siswa</p>
                    </a>
                @endforeach
            </div>
