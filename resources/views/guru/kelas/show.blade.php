<x-layout-dashboard judul-seo="Detail Kelas" :menu="\App\Support\MenuDashboard::guru()">
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <x-kartu>
                <h2 class="font-semibold text-slate-800 dark:text-slate-100">Anggota Kelas ({{ $kelas->anggota->count() }})</h2>
                <div class="mt-3 divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    @empty
                        <p class="py-2 text-sm text-slate-400 dark:text-slate-500">Belum ada siswa. Bagikan kode kelas <strong>{{ $kelas->kode_kelas }}</strong> kepada siswa Anda.</p>
                    @endforelse
                </div>
            </x-kartu>
            ...
            <x-kartu>
                <div class="flex items-center justify-between">
                    <h2 class="font-semibold text-slate-800 dark:text-slate-100">Konten Kelas</h2>
                ...
                <div class="mt-3 divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    @empty
                        <p class="py-2 text-sm text-slate-400 dark:text-slate-500">Belum ada konten yang ditambahkan.</p>
                    @endforelse
                </div>
            </x-kartu>
        ...
        <x-kartu>
            <h2 class="font-semibold text-slate-800 dark:text-slate-100">Info Kelas</h2>
            <dl class="mt-3 space-y-2 text-sm">
                <div><dt class="text-slate-400 dark:text-slate-500">Tahun Ajaran</dt><dd class="font-medium text-slate-700 dark:text-slate-300">{{ $kelas->tahun_ajaran }}</dd></div>
                <div><dt class="text-slate-400 dark:text-slate-500">Deskripsi</dt><dd class="text-slate-600 dark:text-slate-400">{{ $kelas->deskripsi }}</dd></div>
            </dl>
        </x-kartu>
    </div>
    ...
    <x-modal nama="tambah-konten" judul="Tambah Konten Kelas">
        <form method="POST" action="{{ route('guru.kelas.tambah-konten', $kelas) }}" class="space-y-4" x-data="{ jenis: 'e_modul' }">
            @csrf
            <div>
                <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Jenis Konten</label>
                <select name="jenis_konten" x-model="jenis" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3.5 py-2.5 text-sm">
