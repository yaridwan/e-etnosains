<x-layout-dashboard judul-seo="Poster Saya" :menu="\App\Support\MenuDashboard::guru()">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-900">Poster Saya</h1>
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                @foreach($poster as $item)
                    <div class="rounded-xl border border-slate-200 p-3">
                        ...
                        <p class="mt-2 truncate text-sm font-medium text-slate-700">{{ $item->judul }}</p>
                        <div class="mt-2 flex justify-between text-sm">
                            <a href="{{ route('guru.poster.edit', $item) }}" class="font-medium text-teal-700 hover:underline">Ubah</a>
