<x-layout-dashboard judul-seo="Tinjau Pengumpulan Observasi" :menu="\App\Support\MenuDashboard::guru()">
    <a href="{{ route('guru.pengumpulan-observasi.index') }}" class="text-sm text-teal-700 hover:underline">&larr; Kembali</a>
    <h1 class="mt-2 text-2xl font-bold text-slate-900">{{ $pengumpulan->observasi->judul }}</h1>
    <p class="text-sm text-slate-500">Oleh: {{ $pengumpulan->pengguna->nama_lengkap }}</p>

    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <x-kartu class="lg:col-span-2">
            <h2 class="font-semibold text-slate-800">Jawaban</h2>
            <div class="mt-4 space-y-4">
                @foreach($pengumpulan->observasi->butirObservasi as $butir)
                    ...
                    <div class="border-b border-slate-100 pb-3">
                        <p class="text-sm font-medium text-slate-700">{{ $butir->pertanyaan }}</p>
            ...
            @if($pengumpulan->dokumentasi->isNotEmpty())
                <h2 class="mt-6 font-semibold text-slate-800">Dokumentasi</h2>
        ...
        <x-kartu>
            <h2 class="font-semibold text-slate-800">Penilaian</h2>
