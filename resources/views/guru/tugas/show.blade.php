<x-layout-dashboard judul-seo="Detail Tugas" :menu="\App\Support\MenuDashboard::guru()">
    ...
    <p class="text-sm text-slate-500">Kelas: {{ $tugas->kelasBelajar->nama_kelas }}</p>

    <x-kartu class="mt-6">
        <h2 class="font-semibold text-slate-800">Pengumpulan Siswa</h2>
        <table class="mt-4 w-full text-left text-sm">
            <thead class="text-xs uppercase text-slate-400">
                <tr><th class="pb-2">Siswa</th><th class="pb-2">Status</th><th class="pb-2">Nilai</th><th class="pb-2 text-right">Aksi</th></tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                @forelse($tugas->pengumpulan as $pengumpulan)
                    <tr>
                        <td class="py-3 font-medium text-slate-700">{{ $pengumpulan->pengguna->nama_lengkap }}</td>
                        <td class="py-3"><x-badge :warna="$pengumpulan->status === 'dinilai' ? 'emerald' : 'amber'">{{ ucfirst($pengumpulan->status) }}</x-badge></td>
                        <td class="py-3 text-slate-500">{{ $pengumpulan->nilai?->nilai ?? '-' }}</td>
                        <td class="py-3 text-right">
                            <button type="button" x-data @click="$dispatch('buka-modal', 'nilai-{{ $pengumpulan->id }}')" class="text-sm font-medium text-teal-700 hover:underline">Nilai</button>
                        </td>
                    </tr>
                ...
                @empty
                    <tr><td colspan="4"><x-empty-state judul="Belum ada pengumpulan tugas." /></td></tr>
                @endforelse
            </tbody>
        </table>
    </x-kartu>
