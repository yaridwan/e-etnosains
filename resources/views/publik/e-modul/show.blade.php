                ...
                @if(pengaturan_aktif('ulasan_aktif', true))
                    <div class="mt-10">
                        <div class="flex items-baseline justify-between">
                            <h2 class="text-lg font-bold text-slate-900">Ulasan</h2>
                            @if($ulasan->isNotEmpty())
                                <p class="text-sm text-slate-500">
                                    <span class="font-semibold text-amber-600">&#9733; {{ number_format($rataRata, 1) }}</span>
                        ...
                        @auth
                            @if(auth()->id() !== $eModul->id_pengguna)
                                <form method="POST" action="{{ route('e-modul.ulasan.simpan', $eModul) }}" class="mt-4 rounded-2xl border border-slate-200 p-5">
                                    @csrf
                                    ...
                                    <div class="mt-3">
                                        <label for="rating" class="mb-1.5 block text-sm text-slate-600">Rating <span class="text-rose-600">*</span></label>
                                        <select name="rating" id="rating" class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm">
                                    ...
                                    @if($ulasanSaya && $ulasanSaya->status_moderasi === 'menunggu')
                                        <p class="mt-2 text-xs text-amber-700">Ulasan Anda sedang menunggu moderasi Administrator.</p>
                                    @endif
                            ...
                            @forelse($ulasan as $item)
                                <div class="rounded-xl border border-slate-200 p-4">
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm font-semibold text-slate-800">{{ $item->pengguna?->nama_lengkap ?? 'Pengguna' }}</p>
                                        <p class="text-sm text-amber-600">{{ str_repeat('★', $item->rating) }}<span class="text-slate-200">{{ str_repeat('★', 5 - $item->rating) }}</span></p>
                                    </div>
                                    @if($item->komentar)
                                        <p class="mt-2 text-sm text-slate-600">{{ $item->komentar }}</p>
                                    @endif
                                    <p class="mt-1 text-xs text-slate-400">{{ $item->dibuat_pada?->translatedFormat('d F Y') }}</p>
                                </div>
                            @empty
                                <p class="text-sm text-slate-400">Belum ada ulasan untuk E-Modul ini.</p>
                            @endforelse
                        </div>
                    </div>
                @endif
            ...
            <div class="space-y-4">
                ...
                <x-kartu>
                    ...
                    <button type="button"
                        ...
                        class="mt-2 w-full rounded-lg border border-slate-200 py-2 text-sm hover:bg-slate-50">
                        <span x-show="! tersalin">Salin Tautan</span>
                        <span x-show="tersalin" x-cloak class="text-emerald-700">Tautan disalin!</span>
                    </button>
                </x-kartu>

                <x-kartu class="text-center">
                    <h3 class="font-semibold text-slate-800">QR Code</h3>
                    <p class="mt-1 text-xs text-slate-500">Pindai untuk membuka E-Modul ini di perangkat lain.</p>
