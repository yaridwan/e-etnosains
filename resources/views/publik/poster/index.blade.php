<x-layout-publik judul-seo="Poster Edukasi | E-ETNOSAINS">
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold text-slate-900">Poster Edukasi</h1>
        <p class="mt-2 text-slate-500">Galeri poster pembelajaran etnosains.</p>
            ...
            <div class="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                @foreach($poster as $item)
                    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white">
                        ...
                        <p class="truncate p-3 text-sm font-medium text-slate-700">{{ $item->judul }}</p>
                    </div>
                @endforeach
            </div>
