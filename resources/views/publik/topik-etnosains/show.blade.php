<x-layout-publik :judul-seo="$topik->nama_topik.' | E-ETNOSAINS'" :deskripsi-seo="$topik->deskripsi">
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <nav class="text-sm text-slate-500 dark:text-slate-400">
            ...
            <a href="{{ route('topik-etnosains.index') }}" class="hover:text-teal-700 dark:hover:text-teal-400">Topik Etnosains</a> /
            <span class="text-slate-700 dark:text-slate-300">{{ $topik->nama_topik }}</span>
        </nav>

        <h1 class="mt-4 text-3xl font-bold text-slate-900 dark:text-slate-100">{{ $topik->nama_topik }}</h1>
        <p class="mt-2 max-w-2xl text-slate-600 dark:text-slate-400">{{ $topik->deskripsi }}</p>
