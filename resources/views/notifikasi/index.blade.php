...
<x-layout-dashboard judul-seo="Notifikasi" :menu="$menu">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Notifikasi</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Pemberitahuan terkait aktivitas akun dan konten Anda.</p>
        </div>
        ...
        @else
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
                @foreach($notifikasi as $item)
                    <div class="flex items-start justify-between gap-4 py-4 {{ $item->sudahDibaca() ? '' : 'bg-teal-50/40 dark:bg-teal-950/30' }}">
                        <div class="flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-sm text-slate-600 dark:text-slate-400">{{ $item->pesan }}</p>
                                <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">{{ $item->dibuat_pada?->diffForHumans() }}</p>
                            </div>
                        </div>
                        ...
                        @unless($item->sudahDibaca())
                            <form method="POST" action="{{ route('notifikasi.tandai-dibaca', $item) }}">
                                @csrf
                                <button type="submit" class="shrink-0 text-sm font-medium text-teal-700 dark:text-teal-400 hover:underline">Tandai dibaca</button>
                            </form>
                        @endunless
                    </div>
                @endforeach
