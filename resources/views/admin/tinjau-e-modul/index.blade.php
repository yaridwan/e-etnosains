<x-layout-dashboard judul-seo="Tinjau E-Modul" :menu="\App\Support\MenuDashboard::administrator()">
    <h1 class="text-2xl font-bold text-slate-900">Tinjau E-Modul</h1>
    <p class="mt-1 text-sm text-slate-500">Daftar E-Modul yang diajukan guru untuk dipublikasikan.</p>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($eModul as $item)
                    <tr>
                        ...
                        <td class="py-3 text-slate-500">{{ $item->pengguna->nama_lengkap }}</td>
                        <td class="py-3"><x-status-publikasi :status="$item->status_publikasi" /></td>
                        <td class="py-3 text-right">
                            <a href="{{ route('admin.tinjau-e-modul.show', $item) }}" class="text-sm font-medium text-teal-700 hover:underline">Tinjau</a>
                        </td>
                    </tr>
                @empty
