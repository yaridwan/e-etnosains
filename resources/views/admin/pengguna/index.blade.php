<x-layout-dashboard judul-seo="Pengguna" :menu="\App\Support\MenuDashboard::administrator()">
    <h1 class="text-2xl font-bold text-slate-900">Semua Pengguna</h1>

    <form method="GET" class="mt-4 flex flex-wrap gap-3">
        <input type="search" name="q" value="{{ request('q') }}" placeholder="Cari nama/email..." class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm">
        <select name="peran" class="rounded-lg border border-slate-300 px-3.5 py-2 text-sm">
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($pengguna as $item)
                    <tr>
                        <td class="py-3">
                            <p class="font-medium text-slate-700">{{ $item->nama_lengkap }}</p>
                            <p class="text-xs text-slate-400">{{ $item->email }}</p>
                        </td>
                        <td class="py-3 text-slate-500">{{ $item->peran->pluck('nama_peran')->map(fn($p) => ucfirst($p))->implode(', ') }}</td>
                        <td class="py-3"><x-badge :warna="$item->status_akun->value === 'aktif' ? 'emerald' : 'slate'">{{ $item->status_akun->label() }}</x-badge></td>
                        <td class="py-3 text-right">
                            <a href="{{ route('admin.pengguna.show', $item) }}" class="text-sm font-medium text-teal-700 hover:underline">Detail</a>
                        </td>
                    </tr>
                @empty
