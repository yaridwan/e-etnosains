<x-layout-dashboard judul-seo="Halaman Statis" :menu="\App\Support\MenuDashboard::administrator()">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-900">Halaman Statis</h1>
        <x-tombol x-data @click="$dispatch('buka-modal', 'tambah')">Tambah Halaman</x-tombol>
    </div>

    <x-kartu class="mt-6">
        <table class="w-full text-left text-sm">
            <thead class="text-xs uppercase text-slate-400">
                <tr><th class="pb-2">Judul</th><th class="pb-2">Alamat</th><th class="pb-2 text-right">Aksi</th></tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                @forelse($data as $item)
                    <tr>
                        <td class="py-3 font-medium text-slate-700">{{ $item->judul }}</td>
                        <td class="py-3 text-slate-500">/halaman/{{ $item->alamat_tautan }}</td>
                        <td class="py-3 text-right">
                            <button type="button" x-data @click="$dispatch('buka-modal', 'edit-{{ $item->id }}')" class="mr-3 text-sm font-medium text-teal-700 hover:underline">Ubah</button>
                            <x-form-hapus :aksi="route('admin.halaman-statis.destroy', $item)" class="inline" />
                        </td>
                    </tr>

                    <x-modal :nama="'edit-'.$item->id" judul="Ubah Halaman Statis">
                        <form method="POST" action="{{ route('admin.halaman-statis.update', $item) }}" class="space-y-4">
                            @csrf @method('PUT')
                            <x-input label="Judul" name="judul" :value="$item->judul" wajib />
                            <x-textarea label="Konten (HTML)" name="konten" :baris="8" wajib>{{ $item->konten }}</x-textarea>
                            <label class="flex items-center gap-2 text-sm text-slate-600">
                                <input type="checkbox" name="aktif" value="1" @checked($item->aktif) class="rounded border-slate-300 text-teal-700"> Aktifkan
                            </label>
                            <x-tombol type="submit" class="w-full">Simpan Perubahan</x-tombol>
                        </form>
                    </x-modal>
                @empty
                    <tr><td colspan="3"><x-empty-state judul="Belum ada halaman statis." /></td></tr>
                @endforelse
            </tbody>
        </table>
    </x-kartu>

    <x-modal nama="tambah" judul="Tambah Halaman Statis">
        <form method="POST" action="{{ route('admin.halaman-statis.store') }}" class="space-y-4">
            @csrf
            <x-input label="Judul" name="judul" wajib />
            <x-textarea label="Konten (HTML)" name="konten" :baris="8" wajib />
            <x-tombol type="submit" class="w-full">Simpan</x-tombol>
        </form>
    </x-modal>
</x-layout-dashboard>
