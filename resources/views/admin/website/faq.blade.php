        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($data as $item)
                    <tr>
                        <td class="py-3 font-medium text-slate-700">{{ $item->pertanyaan }}</td>
                    ...
                    <x-modal :nama="'edit-'.$item->id" judul="Ubah FAQ">
                        <form method="POST" action="{{ route('admin.faq.update', $item) }}" class="space-y-4">
                            ...
                            <label class="flex items-center gap-2 text-sm text-slate-600">
                                <input type="checkbox" name="aktif" value="1" @checked($item->aktif) class="rounded border-slate-300 text-teal-700">
                                Aktifkan
                            </label>
