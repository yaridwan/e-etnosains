...
<x-layout-dashboard judul-seo="Pengaturan Aplikasi" :menu="\App\Support\MenuDashboard::administrator()">
    <h1 class="text-2xl font-bold text-slate-900">Pengaturan Aplikasi</h1>
    <p class="mt-1 text-sm text-slate-500">Perubahan akan langsung diterapkan ke seluruh tampilan aplikasi.</p>
    ...
    <form method="POST" action="{{ route('admin.pengaturan.simpan') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        ...
        @foreach($kelompok as $namaKelompok => $item)
            <x-kartu>
                <h2 class="font-semibold text-slate-800">{{ $labelKelompok[$namaKelompok] ?? ucfirst($namaKelompok) }}</h2>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    @foreach($item as $pengaturan)
                        @if($pengaturan->tipe === 'boolean')
                            <label class="flex items-center gap-2 text-sm text-slate-600">
                                <input type="hidden" name="{{ $pengaturan->kunci }}" value="0">
                                <input type="checkbox" name="{{ $pengaturan->kunci }}" value="1" @checked($pengaturan->nilai) class="rounded border-slate-300 text-teal-700">
