<x-layout-dashboard judul-seo="Verifikasi Guru" :menu="\App\Support\MenuDashboard::administrator()">
    <h1 class="text-2xl font-bold text-slate-900">Verifikasi Guru</h1>
    <p class="mt-1 text-sm text-slate-500">Tinjau dan setujui pendaftaran akun guru baru.</p>
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($verifikasi as $item)
                    <tr>
                        <td class="py-3">
                            <p class="font-medium text-slate-700">{{ $item->pengguna->nama_lengkap }}</p>
                            <p class="text-xs text-slate-400">{{ $item->pengguna->email }}</p>
                        </td>
                        <td class="py-3 text-slate-500">{{ $item->pengguna->profilGuru?->instansiPendidikan?->nama_instansi }}</td>
                        <td class="py-3">
                        ...
                        <td class="py-3 text-right">
                            <a href="{{ route('admin.verifikasi-guru.show', $item) }}" class="text-sm font-medium text-teal-700 hover:underline">Tinjau</a>
                        </td>
                    </tr>
                @empty
